...
class TERM_TUI:
    ...
    def draw_table(self):
        ...
        self.safe_addstr(1, 2, "VISUAL MATRIX (Arrows to Navigate)", curses.color_pair(PAIR_RED)|curses.A_BOLD)
        
        # Simplified Grid Layout (Z values)
        layout = [
            ...
            [], # Gap
            [0,0,0,57,58,59,60,61,62,63,64,65,66,67,68,69,70,71], # Lanthanides (Shifted for visual fit)
            [0,0,0,89,90,91,92,93,94,95,96,97,98,99,100,101,102,103] # Actinides
        ]
        ...
        for r_idx, row in enumerate(layout):
            ...
            for c_idx, z in enumerate(row):
                if z == 0:
                    self.safe_addstr(start_y + r_idx, 2 + (c_idx*3), "   ")
                else:
                    # Find element symbol
                    # This is inefficient but functional for small dataset
        ...
        # Info Box at bottom
        info_y = start_y + len(layout) + 1
        curr_name = self.elements[self.selected_idx]['name'].upper()
        self.safe_addstr(info_y, 2, f"SELECTED: {curr_z} - {curr_name}", curses.color_pair(PAIR_BRASS))
    ...
    def loop(self):
        while True:
            ...
            elif self.view_mode == "TABLE": self.draw_table()
