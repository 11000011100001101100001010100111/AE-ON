...
class TERM_TUI:
    ...
    # --- HELPERS ---
    def get_prop(self, props, keys):
        """Fuzzy search for a property value."""
    ...
    def draw_inspector(self):
        ...
        # 1. Header Info (Z and Name)
        name = data.get("name", "Unknown").upper()
        self.safe_addstr(1, 2, f" {z:03d} | {name} ", curses.color_pair(PAIR_RED) | curses.A_BOLD)

        y = 3
        
        # 2. Critical Stats (Forced Top)
        mass = self.get_prop(props, ['weight', 'mass'])
        self.safe_addstr(y, 2, "ATOMIC MASS:", curses.color_pair(PAIR_GREY) | curses.A_BOLD)
        self.safe_addstr(y, 16, mass, curses.color_pair(PAIR_SILVER))
        y += 1
        
        phase = self.get_prop(props, ['phase', 'state'])
        self.safe_addstr(y, 2, "PHASE:", curses.color_pair(PAIR_GREY) | curses.A_BOLD)
        self.safe_addstr(y, 16, phase, curses.color_pair(PAIR_SILVER))
        y += 2
        ...
        # 4. Description (The new Text Block)
        summary = data.get("summary", "No data.")
        self.safe_addstr(y, 2, "DESCRIPTION:", curses.color_pair(PAIR_BRASS))
        y += 1
        
        # Calculate remaining height for text
        text_height = (self.height - 3) - y
        if text_height > 0:
            lines = textwrap.wrap(summary, self.width - 4)
            for i, line in enumerate(lines):
                if i >= text_height: break
                self.safe_addstr(y + i, 2, line, curses.color_pair(PAIR_SILVER))
    ...
    def draw_compare(self):
        ...
        def print_col(data, z, start_x, max_w):
            ...
            for label, keywords in targets:
                val = self.get_prop(props, keywords)
