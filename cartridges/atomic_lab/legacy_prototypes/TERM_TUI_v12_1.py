...
class TERM_TUI:
    ...
    # --- SPLASH SCREEN ---
    def run_splash(self):
        """Iterates Z=1 to 118 in a centered atomic animation."""
        cy = self.height // 2
        cx = self.width // 2
        ...
        # Animation Loop
        # We skip frames if needed to keep boot fast, or just run fast
        # 118 frames at 0.015s = ~1.8 seconds.
        for z in range(1, 119):
            # Clear atom area (rough box clear)
            ...
            for i, count in enumerate(shells):
                ...
                if count > 0:
                    for e_idx in range(count):
                        # Rotating spin effect based on Z
                        offset = (z * 15) 
        ...
        time.sleep(0.5) # Pause at full stable 118
