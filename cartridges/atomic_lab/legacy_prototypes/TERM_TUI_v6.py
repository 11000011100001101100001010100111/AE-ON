...
PAIR_ELECTRON = 7  # Cyan for electrons
PAIR_NUCLEUS = 8   # White/Bold for Nucleus
...
class TERM_TUI:
    def __init__(self, stdscr):
        ...
        try:
            ...
            curses.init_pair(PAIR_ELECTRON, 51, -1) # Cyan/Neon Blue
            curses.init_pair(PAIR_NUCLEUS, 15, -1)  # Bright White
        except:
            # Fallback
    ...
    def draw_inspector(self):
        ...
        if config:
            ...
            self.safe_addstr(y, 10, config, curses.color_pair(PAIR_ELECTRON))
            y += 2
    ...
    def draw_compare(self):
        ...
        if not d1 or not d2: 
            self.safe_addstr(self.height//2, 2, "DATA ERROR", curses.color_pair(PAIR_RED))
            return
        ...
        def print_col(data, start_x, max_w):
            ...
            for label, keywords in targets:
                ...
                if len(val_str) > max_w: val_str = val_str[:max_w-1] + "…"
    ...
    def draw_lab(self):
        """Dynamic Atom Visualizer and Editor."""
        cy = self.height // 2 - 2
        cx = self.width // 2
        
        # 1. Identify Element based on Z
        ...
        # Stats
        mass_num = self.lab_z + self.lab_n
        charge = self.lab_z - self.lab_e
        
        # Stability Heuristic (Very basic)
        stable = "STABLE"
        ratio = self.lab_n / self.lab_z if self.lab_z > 0 else 0
        if self.lab_z > 83 or ratio < 1.0 or ratio > 1.6:
            stable = "UNSTABLE"
        ...
        # 3. Draw Atom Visual (Bohr Modelish)
        # Nucleus
        ...
        # Electron Shells logic
        # Max capacity: 2, 8, 18, 32...
        shells = []
        rem_e = self.lab_e
        ...
        for i, count in enumerate(shells):
            radius = 3 + (i * 2)
            # Draw simplistic electrons around the center
            angle_step = 360 / count if count > 0 else 0
            for e_idx in range(count):
                theta = math.radians(e_idx * (360/count) + (i*45))
                ex = cx + int(radius * 2 * math.cos(theta)) # X aspect ratio correction
        ...
        self.safe_addstr(help_y+1, 2, "←/→: Protons (Z)   ↑/↓: Neutrons (N)", curses.color_pair(PAIR_SILVER))
        self.safe_addstr(help_y+2, 2, "[ / ]: Electrons (e)   q: Exit Lab", curses.color_pair(PAIR_SILVER))
        ...
        while True:
            ...
            if self.drawer_open:
                if c == ord('s'): 
                    ...
                    # Init Lab with current selection
                    sel = self.elements[self.selected_idx]
                    self.lab_z = sel['z']
                    self.lab_n = int(sel['z'] * 1.1) # Approx neutron guess
                    self.lab_e = sel['z']
                    self.view_mode = "LAB"
                    self.drawer_open = False
                elif c == ord('e'): 
