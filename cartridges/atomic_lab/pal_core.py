...
class TERM_TUI:
    def __init__(self, stdscr):
        ...
        self.layout = [
            ...
            [], # Vertical Gap (Index 7)
            [0,0,0,57,58,59,60,61,62,63,64,65,66,67,68,69,70,71], # Lanthanides
    ...
    def find_grid_neighbor(self, curr_z, dr, dc):
        # 1. Find coords
        cr, cc = -1, -1
    ...
    def loop(self):
        while True:
            ...
            if c == curses.KEY_UP:
                ...
                elif self.view_mode == "BROWSE": self.selected_idx = max(0, self.selected_idx - 1)
                elif self.view_mode == "TABLE":
                    curr_z = self.elements[self.selected_idx]['z']
                    new_z = self.find_grid_neighbor(curr_z, -1, 0)
                    for i, el in enumerate(self.elements):
                        if el['z'] == new_z: self.selected_idx = i; break

            elif c == curses.KEY_DOWN:
                if self.view_mode == "INSPECT": self.inspector_scroll += 1
                elif self.view_mode == "BROWSE": self.selected_idx = min(len(self.elements)-1, self.selected_idx + 1)
                elif self.view_mode == "TABLE":
                    curr_z = self.elements[self.selected_idx]['z']
                    new_z = self.find_grid_neighbor(curr_z, 1, 0)
                    for i, el in enumerate(self.elements):
            ...
            if self.drawer_open:
                if c == ord('s'): self.view_mode = "SEARCH"; self.drawer_open = False
                    ...
                    curses.endwin()
                    print("\n[SYSTEM] Initializing DeepNet Harvester Protocol...")
                    try:
                        bot = TERM_Harvester()
                        bot.run()
                        input("\n[SYSTEM] Update Complete. Press Enter to Reboot...")
                    except Exception as e:
                        print(f"\n[ERROR] Harvester failure: {e}")
